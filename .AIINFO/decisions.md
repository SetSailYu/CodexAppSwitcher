...
最近更新：2026-07-13 09:19 (Asia/Shanghai)

## 2026-07-13 09:19 (Asia/Shanghai) - 用量窗口兼容策略

背景：官方 Codex 用量接口可能不再返回旧短期额度窗口，只返回按日期聚合的历史用量数组。

候选方案：继续把缺失窗口视为失败；根据历史用量推算额度；将额度窗口视为可选展示。

最终选择：额度窗口改为可选展示，旧 `rate_limit.primary_window/secondary_window` 继续解析；仅返回历史用量时显示 `--` 和 `未提供`。

原因：历史用量不能等价推算剩余额度或重置时间，伪造百分比会误导用户。

影响范围：用量解析服务、账号列表、桌面挂件、README、CHANGELOG 和版本号。
