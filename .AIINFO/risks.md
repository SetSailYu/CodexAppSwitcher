...
## 2026-07-13 09:19 (Asia/Shanghai) - 官方用量接口结构继续变化

描述：`wham/usage` 已出现只返回历史用量、不返回额度窗口的结构，后续字段名或授权要求仍可能变化。

影响：用量刷新可能再次返回可读 JSON 但无法映射到剩余额度和重置时间。

当前应对策略：解析器只在存在明确额度窗口时展示百分比；缺失窗口时显示未提供，不根据历史用量推算额度。后续排查优先查看响应预览和字段结构。
