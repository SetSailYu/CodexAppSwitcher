...
阶段四：体验完善  
进行中。主窗口两栏布局、账号列表固定操作列、当前账号整行高亮、桌面额度悬浮球、托盘后台入口和托盘菜单优化均已完成。当前版本已定为 `v1.1.0`，项目元数据、manifest、主窗口版本显示、`README.md`、`CHANGELOG.md` 和 README 界面预览截图均已同步；VS Release/win-x86 发布错误已修复并验证通过；主窗口顶部系统白色边框已通过 WindowChrome 配置去除；README 已替换为最新脱敏 UI 截图并补充托盘菜单展示。
...
- UI 信息架构：左侧栏已取消，主内容区和右侧状态详情已完成；账号列表固定操作列、桌面额度悬浮球、球内手动刷新、外圈进度显示、托盘后台状态入口、托盘右键菜单图标优化、托盘 Logo 图标、菜单顶部状态文案移除、主窗口顶部白色边框去除和 README 脱敏截图更新已完成。
- v1.1.0 定版：项目版本元数据、窗口版本显示、README、CHANGELOG 和发布配置兼容性已完成，待用户手动验证。
