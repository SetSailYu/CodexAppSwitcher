...
当前版本采用固定策略：关闭 Codex App，写入目标账号的 `%USERPROFILE%\.codex\auth.json`，再自动启动 Codex App。切换流程不处理 token、不替换 Roaming/MSIX 状态，也不隔离历史会话。用量刷新会在账号独立 WebView2 登录态中临时读取前端会话授权并请求 ChatGPT 用量接口，但不会展示、导出或保存 token。
...
- 刷新用量：通过账号独立 WebView2 登录态请求 ChatGPT `wham/usage` 接口并解析 5 小时/每周用量。
...
- 用量刷新只在 WebView2 页面上下文中临时使用 ChatGPT 前端会话授权，不保存 token。
- 不创建 rollback。
...
- 分别刷新每个账号的用量，确认操作日志显示 `HTTP 200` 且额度同步更新。
...
优先点击该账号行的 `Web`，重新打开 ChatGPT Web 登录窗口并确认登录完成，然后再刷新用量。当前版本会从 `/api/auth/session` 临时读取前端会话授权，请求 `backend-api/wham/usage`；如果仍失败，操作日志会显示 HTTP 状态、session 状态和响应预览。
